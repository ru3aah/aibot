...
def _postgres_sync_url() -> str:
    pg = getattr(settings, "POSTGRES_URL", None)
    if not pg:
        raise RuntimeError("POSTGRES_URL is not set, but DB_MODE=postgres")
    url = make_url(str(pg))
    return _render_url(url.set(drivername="postgresql+psycopg"))
...
def init_engines_sync() -> None:
    ...
    try:
        ...
        # running event loop (aiogram, etc.)
        # ВАЖНО: если DB_MODE=postgres — НЕЛЬЗЯ падать в sqlite.
        if getattr(settings, "DB_MODE", "sqlite") == "postgres":
            sync_url = _postgres_sync_url()
        else:
